...
Si $\alpha = \alpha_{2}$ (en jaune) alors \textbf{$|R_{\alpha_{2}}| > R_T$} et le point b appartient au disque généralisé de rayon $R_{\alpha_{2}}$ ( b appartient au complémentaire du disque de rayon $R_{\alpha_{2}}$). On ne sait pas encore si le convergent c sera un sommet de l'$\alpha$-shape, mais on sait que b n'en sera pas un. Nous pouvons poursuivre le calcul des convergents.\\

Si $\alpha = \alpha_{1}$ (en violet) alors \textbf{$|R_{\alpha_{1}}| < R_T$} et le point b n'appartient pas à notre disque généralisé de rayon $R_{\alpha_{1}}$. L'$\alpha$-hull ne peut rejoindre c par a sans au moins passer par b. b et c appartiennent à l'$\alpha$-shape. Il faut de plus vérifier si les points $b_i = p_{k-2} + i*p_{k-1} \forall i \in [0, q_k-2]$ appartiennent également à l'$\alpha$-shape. \\

De part la construction des triangles, la taille des rayons de leur cercle circonscrit $R_{T_{i}}$ est croissante (Un exemple probant est donné \textsc{figure} \ref{fig:nas-dicho}). Il suffit de tester le dernier, avec le plus grand cercle pour savoir si nous pouvons continuer l'algorithme avec la recherche des convergents suivants ou si nous devons déterminer le point $\alpha$-extrême par une recherche dichotomique. La recherche dichotomique permet de trouver en $log(q_k)$ le triangle adéquate $T_i = (a, b_{i}, b_{i+1}$ tel que $R_{T_i} > R_{\alpha}$ et $R_{T_{i-1}} \leq R_{\alpha}$ où $q_k$ est le coefficient trouvé par le calcul du convergent $p_k = p_{k-2} + q_k * p_{k-1}$.
...
La méthode de calcul de l'$\alpha$-shape pour $\alpha > 0$ est fondamentalement différente des méthodes de calculs présentées précédemment. La méthode n'est plus incrémentale mais de type ``Bottom-Up''. L'algorithme part du cas critique de l'enveloppe convexe. L'ensemble des sommets de l'$\alpha$-shape est un sous-ensemble de l'enveloppe convexe. 

L'algorithme commence différemment des autres déjà présentés. Le départ est en deux phases. Il faut d'une part récupérer un point de départ pertinent mais également l'ensemble des sommets de l'enveloppe convexe. Le point d'ordonnée minimale et d'abscisse maximale n'est plus nécessairement dans l'$\alpha$-shape et produirait un un calcul potentiellement faux. Pour être certain de notre départ, nous ne le calculons pas mais le choisissons par construction parmi l'un des trois sommets du triangle composant le cercle circonscrit. Ensuite, nous récupérons à partir de ce point l'ensemble des sommets de l'enveloppe convexe du disque discret par l'intermédiaire de notre algorithme ``Output Sensitive''. On note $\mathcal{S}$ cet ensemble.
...
Notons a,b et c trois sommets successifs appartenant à S. Supposant que a appartient à l'$\alpha$-shape, nous vérifions la présence de b et c dans l'$\alpha$-shape. Nous utilisons un prédicat qui compare la taille du rayon \textbf{$R_T$} du cercle circonscrit au triangle : $T(a, b, c)$ à la taille du rayon de notre disque généralisé : \textbf{$R_{\alpha}$} $= 1/\alpha$. Il faut distinguer deux cas de figures.\\
...
Si $\alpha = \alpha_{1}$ (en violet) alors \textbf{$R_{\alpha_{1}} < R_T$}. Le point b appartient au disque généralisé de rayon $-1/R_{\alpha_{1}}$. Il est possible de rejoindre c par a via un arc de cercle dont le disque comprend l'ensemble des points de $\mathcal{D}$. b appartient à l'$\alpha$-hull, mais n'appartient pas à l'$\alpha$-shape. On continue la procédure pour étudier les sommets suivants. a reste inchangé, b devient c et c devient le sommet suivant de $\mathcal{S}$.
...
Si $\alpha = \alpha_{2}$ (en jaune) alors \textbf{$R_{\alpha_{2}} > R_T$}. Le point b n'appartient pas au disque généralisé de rayon $1/R_{\alpha_{1}}$. L'$\alpha$-hull ne peut rejoindre c par a sans au moins passer par b. \textbf{b appartient à l'$\alpha$-shape}. Il faut poursuivre la procédure afin de réaliser un tour complet. b devient a, c devient b et c devient le sommet suivant de $\mathcal{S}$.\\
...
La version présente de l'algorithme s'appuie sur l'enveloppe convexe selon la méthode de Har-Peled qui récupère les sommets en $O(R^{2/3}_{D})$. On parcourt cette liste une seule fois en éliminant des sommets potentiels ou en ajoutant le sommet à l'alpha-shape. 
